"""
Audio one-shots: press 1/2/3 in game mode to play the bundled sounds.

Builds the scene in its own level (Diorama 15-audio how-to: Docs/howto/15-audio.md).
...
def log(msg):
    print("DIORAMA_AUDIO: {}".format(msg))
...
INPUT_CONFIG = {
    "actions": [
        {"name": "blip", "kind": 0, "bindings": [{"channel": "keyboard_key_alphanumeric_1", "scale": 1.0, "axis": 0}]},
        {"name": "hit", "kind": 0, "bindings": [{"channel": "keyboard_key_alphanumeric_2", "scale": 1.0, "axis": 0}]},
        {"name": "jump", "kind": 0, "bindings": [{"channel": "keyboard_key_alphanumeric_3", "scale": 1.0, "axis": 0}]},
    ]
}
...
def main():
    ...
    if sprite is None or inp is None:
        log("FAIL: components not found (rebuild/enable the gem?)")
        return

    # One visible entity carrying the input map and the one-shot script.
    player, _ = make_entity("SoundBoard", math.Vector3(0.0, 0.0, 1.0), [sprite, inp])
    diorama.DioramaSpriteRequestBus(bus.Event, "SetTextureByPath", player, "diorama/textures/o3de_mascot.png")
    diorama.DioramaSpriteRequestBus(bus.Event, "SetSize", player, 4.0, 5.1)
    ...
    if general.get_current_level_name() == LEVEL_NAME:
        try:
            general.save_level()
            patch_prefab(LEVEL_NAME, {'SoundBoard': ['diorama/examples/audio/audio_oneshot.luac']}, input_config=INPUT_CONFIG, extra=None)
        except Exception as e:
    ...
    log("Audio demo built in " + LEVEL_NAME)
    log("MANUAL: enter game mode and press 1 / 2 / 3 to hear blip / hit / jump")
    log("        through DioramaAudioRequestBus.PlayOneShot (MiniAudio underneath).")
    log("done")
